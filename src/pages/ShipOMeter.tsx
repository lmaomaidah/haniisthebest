...
const ShipOMeter = () => {
  ...
  const updateTrait = (personNum: 1 | 2, traitId: string, value: number) => {
    ...
    setter(prev => prev.map(t => t.id === traitId ? {
      ...t,
      value
    } : t));
  };
  const PersonCard = ({
    ...
    setPerson: (p: ImageType | null) => void;
    label: string;
      ...
      {person ? <div className="flex flex-col items-center gap-4">
          {person.image_url ? <div className="w-32 h-32 rounded-2xl overflow-hidden border-4 border-primary shadow-glow">
              <img src={person.image_url} alt={person.name} className="w-full h-full object-cover" />
            </div> : <div className="bg-gradient-to-br from-primary via-secondary to-accent rounded-2xl px-6 py-4 border-4 border-primary shadow-glow">
        ...
        </div> : <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto">
          {images.map(img => <button key={img.id} onClick={() => setPerson(img)} className="p-2 rounded-xl hover:bg-primary/20 transition-colors">
              {img.image_url ? <img src={img.image_url} alt={img.name} className="w-12 h-12 rounded-lg object-cover mx-auto" /> : <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-xs font-bold text-primary-foreground">
                  {img.name.slice(0, 2)}
                </div>}
      ...
      <div className="flex gap-4 justify-center text-muted bg-background">
        {[0, 1].map(i => <div key={i} className="flex flex-col items-center gap-2">
            ...
            <div className="h-32 w-8 bg-muted rounded-full overflow-hidden relative">
              <div className="absolute bottom-0 w-full bg-gradient-to-t from-primary to-secondary transition-all" style={{
            height: `${values[i]}%`
          }} />
            </div>
            <input type="range" min="0" max="100" value={values[i]} onChange={e => {
        ...
        {person1 && person2 && <>
            ...
            <div className="backdrop-blur border-4 border-secondary/30 rounded-3xl p-6 mb-8 bg-card/50">
              ...
              <div className="grid md:grid-cols-2 gap-x-12 gap-y-6">
                {defaultTraits.map(trait => <div key={trait.id} className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Slider value={[person1Traits.find(t => t.id === trait.id)?.value || 50]} onValueChange={([v]) => updateTrait(1, trait.id, v)} max={100} step={1} className="flex-1" />
                    </div>
            ...
            {/* Calculate Button */}
            <div className="text-center mb-10">
              <Button onClick={calculateCompatibility} size="lg" className="gradient-pink-blue text-white text-2xl px-12 py-8 rounded-3xl shadow-glow hover:scale-110 transition-transform">
            ...
            {/* Result */}
            {showResult && compatibility !== null && <div className="bg-card/50 backdrop-blur border-4 border-pink-400/50 rounded-3xl p-8 text-center animate-bounce-in">
                ...
                <div className="relative w-48 h-48 mx-auto mb-6">
                  ...
                  <div className="absolute inset-0 rounded-full border-8 border-pink-400 transition-all duration-1000" style={{
